...
<div wire:poll.1000ms> {{-- poll suave para detectar cambios y pausas --}}
    @if (!$current)
        <div class="alert alert-info">Esperando pregunta…</div>
        @if (!$gameSession->is_running && !$gameSession->is_active)
            <a class="btn btn-outline-secondary btn-sm" href="{{ route('winners', $gameSession) }}">Ver ganadores</a>
        @endif
        @return
    @endif
    ...
    <div x-data="{
        seconds: {{ (int) $timer }},
        running: {{ $gameSession->is_paused ? 'false' : 'true' }},
    ...
    setInterval(() => tick(), 1000);" class="card">
        <div class="card-body">
            ...
            <div class="list-group">
                @foreach ($current->question->options->sortBy('opt_order') as $opt)
                    ...
                    {{ $answered_option_id === $opt->id ? 'active' : '' }}"
                        :disabled="answered || {{ $gameSession->is_paused ? 'true' : 'false' }}"
                        @click="if(!answered && !{{ $gameSession->is_paused ? 'true' : 'false' }}){answered = true;$wire.answer({{ $opt->id }}, elapsed());}">
                        <div><strong>{{ $opt->label }}.</strong> {{ $opt->content }}</div>

                        {{-- Cuando el docente pausa (revelar), mostramos la correcta --}}
            ...
            @if ($answered_option_id && !$gameSession->is_paused)
                <div class="alert alert-secondary mt-3 mb-0">Respuesta registrada. Espera a que finalicen todos…</div>
            @endif
        </div>
    </div>
</div>
